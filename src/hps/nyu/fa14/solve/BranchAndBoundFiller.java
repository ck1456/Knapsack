...
public class BranchAndBoundFiller extends AbstractFiller {
    ...
    private final Queue<BranchNode> nodesToExplore = new PriorityQueue<BranchNode>(20000, RANK_BY_UPPER_BOUND_DESC);
    ...
    private void exploreNode(BranchNode p) {
        ...
        { // left include
            ...
            if (c.currentWeight <= k.capacity) { // only go any further if feasible
                ...
                c.upperBound = updateUpperBound(c.currentValue, k.capacity - c.currentWeight, c.level);
                if(c.upperBound > bestValue){ // Only add if the upperbound is feasibly better
    ...
    private void updateBestConstraints(BranchNode c){
        if(c.currentValue > bestValue){
            ...
            System.out.println(String.format("Best Value: %d Constraints: %d Queue: %d UpperBound: %d",
                    bestValue, c.constraints.length, nodesToExplore.size(), c.upperBound));
        }
    }
    
    // calculate upper bound based on Greedy solution
    private int updateUpperBound(int currentValue, int remainingCapacity, int index) {
        ...
        while (remainingCapacity > 0 && i < rankedItems.size()) {
            ...
            int amount = (int) Math.ceil(item_i.value
                    * Math.min(1.0, (double) remainingCapacity / item_i.weight));
